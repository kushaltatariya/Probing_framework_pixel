...
class ConlluUDParser:
    ...
    def find_category_token(
        ...
        """
        ##### CHECKS TO SEE HOW TO FIX ROOT ISSUE #####
        # if category == 'Tense':
        #     if head["form"] == 'populated':
        #         print(head['feats'])
        #         if category in head['feats']:
        #             print('YESSS - for populated')
        #     if head["form"] == 'Blair':
        #         # print([(child.token, child.token['feats']) for child in children])

            # siblings = []
            # for child in children:
            #     if child.token['feats']:
            #         if category in child.token['feats']:
            #             siblings.append(child.token)
            # if len(siblings) > 1:
            #     print(siblings)
            #     print(head)
            #     print('Yo')
        ...
        #         print(type(category))
        #         print(children[2].token['feats'])
        #         if children[2].token['feats']:
        #             print('Yo')
        #             if category in children[2].token['feats']:
        #                 print('YESSS - for is')
        ...
        if head["feats"] and category in head["feats"]:
            ...
            for token in children:
                if token.token["feats"] and category in token.token["feats"]:
                    if (
                        self.upos
                        and token.token["upos"] == self.upos
                        and self.deprel
                        and token.token["deprel"] == self.deprel
                    ):
                        return token.token
                    elif self.deprel and token.token["deprel"] == self.deprel:
                        return token.token
                    elif self.upos and token.token["upos"] == self.upos:
                        return token.token
                    elif not self.upos and not self.deprel:
                        return token.token
    ...
    def classify(
        ...
        for token_tree in token_trees:
            if token_tree.metadata:
                ...
                # if category == "Tense" and root['upos'] != 'VERB':
                #     with open('tense_not_verb.txt', 'a') as f:
                #         f.write(s_text + '\n')
